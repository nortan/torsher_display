...
ТВ-браузеры слабые, поэтому в плеере соблюдаются правила:

- **Рисуется только то, что видно.** Слайды, которые не участвуют в переходе, получают
  `visibility: hidden; content-visibility: hidden` (атрибут `data-lean` на `#stage`). Без этого каждое чтение
  стилей в GSAP (`getComputedStyle` при запуске анимаций) пересчитывало стили и раскладку всех слайдов.
  Для «Колоды карт» видимы ещё три карты за текущей. `fitAll()` на время замера снимает `data-lean`.
- **GPU-слои только у активного слайда:** `will-change` задан через `.swiper-slide-active …`, а не глобально.
- **Тень фото не пересчитывается на каждом кадре.** Анимируется обёртка `.plate__in`, а `drop-shadow` и маска
  стоят на статичной `<img>` внутри неё.
- **Бесконечные эффекты анимируют только `transform` и `opacity`.** «Свечение» цены — это копия текста
  с готовой тенью (`.price-glow`), у которой мигает прозрачность. Не анимируйте в цикле `filter`, `text-shadow`,
  `box-shadow`, размеры и фон.
- Фото следующего слайда декодируются заранее (`img.decode()`), чтобы декодирование не попадало на первый кадр перехода.

Замер: Chromium с замедлением CPU в 4 раза (`Emulation.setCPUThrottlingRate`), частота кадров по `requestAnimationFrame`
за 20–40 с при смене слайдов каждые 4 с. После оптимизации: «Затухание» — 20 → 55 кадров/с, число
слоёв — 329 → 63; «Куб» и «Переворот» — 1,6 → 33–39; «Колода карт» — 2 → 20.
