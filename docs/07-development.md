...
Параметры `?v=` в `index.html` и `admin/index.html` проставляет `tools/build.mjs` (функция `stamp`).
В исходниках репозитория их нет, и руками их добавлять не нужно. Плеер регистрирует только те
плагины GSAP, которые действительно загрузились, поэтому устаревший `index.html` из кеша
не ломает экран.
